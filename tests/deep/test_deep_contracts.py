...
def test_planning_stays_deterministic_across_many_valid_names() -> None:
    for index in range(100):
        ...
        first = kernform.plan_project(name=name, signatures=(kernform.Signature.SDK,))
        second = kernform.plan_project(name=name, signatures=(kernform.Signature.SDK,))
...
def test_parallel_planning_stress_is_byte_equivalent() -> None:
    def plan(_: int) -> str:
        return kernform.plan_project(
            name="parallel-contract", signatures=(kernform.Signature.SDK,)
        ).json
...
def test_planner_performance_canary() -> None:
    ...
    for index in range(1000):
        kernform.plan_project(name=f"perf-{index}", signatures=(kernform.Signature.SDK,))
